...
TEXTS = {
    "Indonesia": {
        "home": "Beranda",
        "home_title": "Database Operator United Tractors",
        "home_desc": "Aplikasi ini memudahkan Anda untuk mencari, memfilter, dan mengunduh data karyawan secara cepat dan akurat.",
...
if menu == TEXTS[lang]["home"]:
    try:
        ...
        st.markdown(
            f"""<video autoplay muted loop playsinline class="video-bg">
                <source src="data:video/mp4;base64,{video_b64}" type="video/mp4"></video>
                <div class="overlay"></div>""",
            unsafe_allow_html=True
        )
    except Exception as e:
        st.error(f" Video gagal dimuat: {e}")

    st.markdown("<div style='margin-left: 50px;'>", unsafe_allow_html=True)
    try:
        st.image("United-Tractors-New-Thumbnail1_imgupscaler.ai_V1(Fast)_2K.png", width=530)
    except:
        pass
...
# -------------------------
    ...
    try:
        ...
        col1, col2 = st.columns([2, 1])
        with col1:
            search_name = st.text_input("Cari nama").strip().lower()
        with col2:
